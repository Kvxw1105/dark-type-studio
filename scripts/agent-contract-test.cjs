...
for (const [format, signature] of [["png", "89504e47"], ["jpg", "ffd8ff"], ["webp", "52494646"]]) {
  ...
  const exported = spawnSync(process.execPath, ["bin/dark-type.cjs", "export", projectPath, "--format", format, "--output", output], { cwd: path.resolve(__dirname, ".."), encoding: "utf8", timeout: 120000 });
...
const mcp = spawnSync(process.execPath, ["mcp-server.cjs"], { cwd: path.resolve(__dirname, ".."), input: mcpInput, encoding: "utf8", timeout: 120000 });
